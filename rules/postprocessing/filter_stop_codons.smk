rule filter_internal_stop_codons:
    ...
    """
    input:
        merged_gtf = "output/{sample}/augustus.hints.fixed.gtf",
        genome = lambda w: os.path.join(get_output_dir(w), "genome.fa")
    output:
        filtered_gtf = "output/{sample}/galba.filtered.gtf",
    ...
    shell:
        ...
        echo "[INFO] Input GTF: {input.merged_gtf}" | tee -a {output.filter_log}
        ...
        python3 {script_dir}/getAnnoFastaFromJoingenes.py \
            -g {input.genome} \
            -f {input.merged_gtf} \
            -o {params.output_dir}/galba.check \
        ...
        STEM="{params.output_dir}/galba.check"
...
PYEOF
            ...
            cp {input.merged_gtf} {output.filtered_gtf}
        fi
        ...
        test -f "{params.output_dir}/bad_genes.lst" && {{
            ...
            mv -f {params.output_dir}/bad_genes.lst {params.output_dir}/bad_genes.galba.lst
        ...
        test -f "bad_genes.lst" && {{
            ...
            mv -f bad_genes.lst {params.output_dir}/bad_genes.galba.lst
        }} || true
