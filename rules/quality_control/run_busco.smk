...
rule busco_genome:
    ...
    input:
        genome=lambda wildcards: os.path.join(get_output_dir(wildcards), "genome.fa")
    output:
...
rule get_longest_isoform:
    ...
    output:
        gtf="output/{sample}/galba.longest.gtf",
        aa="output/{sample}/galba.longest.aa"
    log:
...
rule busco_proteins:
    ...
    input:
        proteins="output/{sample}/galba.longest.aa"
    output:
