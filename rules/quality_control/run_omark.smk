...
# Fall back to shared_data/LUCA.h5 when the user did not set [OMARK] omamer_db.
# Explicit `or` handles the case where the Snakefile set the config entry
# to None/False (fallback sentinel in ConfigParser.get).
OMAMER_DB = config.get("omamer_db") or "shared_data/LUCA.h5"
...
rule generate_splice_file:
    ...
    shell:
        ...
        # Extract gene_id -> transcript_id mapping from galba.gtf
...
rule run_omark:
    ...
    shell:
        ...
        # access that download fails. If a pre-downloaded taxdump.tar.gz was
        # staged via download_data.sh --omark it is available as {input.taxdump}.
        # Copy it into ~/.etetoolkit/ so ete3 converts it locally instead of
        # attempting a network fetch.
        #
        # Serialise via flock on ~/.etetoolkit/taxa.sqlite.lock so that
        # concurrent samples (or concurrent pipeline runs) do not collide when
