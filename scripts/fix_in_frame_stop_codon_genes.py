...
# Author: Anica Hoppe & Katharina Hoff
# Last modified: October 12th 2025
...
parser.add_argument('-w', '--workdir', required=False, type=str, default='.',
                    help = "Set working directory for temporary files and logs. Default is current directory.")
...
def create_random_string(workdir='.'):
    ...
    tmp = os.path.join(workdir, "tmp_" + randomString)
    log = os.path.join(workdir, "fix_IFS_log_" + randomString)
...
def create_log_file_name(randomString, workdir='.'):
    """ Function that creates a log file with a random name """
    log = os.path.join(workdir, "fix_IFS_log_" + randomString)
    return(log)


def create_tmp_dir(randomString, workdir='.'):
...
if not os.path.exists(workdir):
    os.makedirs(workdir)
rString = create_random_string(workdir)
log = create_log_file_name(rString, workdir)
...
tmp = create_tmp_dir(rString, workdir)
