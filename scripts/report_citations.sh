...
cite() {
    ...
    # Skip if this key was already cited (avoid duplicates)
    local marker_file="$report_dir/.cited_keys"
    touch "$marker_file"
    if grep -qx "$key" "$marker_file" 2>/dev/null; then
        if command -v flock &>/dev/null; then true; fi
        exit 0
    fi
    echo "$key" >> "$marker_file"
...
BIBEOF
            ;;
        galba)
            echo "Hoff, K. J., Bruna, T., Lomsadze, A., Borodovsky, M., & Stanke, M. (2023). A pipeline for automated gene prediction for novel species using protein homology. NAR Genomics and Bioinformatics, 5(3), lqad064. doi:10.1093/nargab/lqad064" >> "$txt_file"
            cat >> "$bib_file" << 'BIBEOF'
@article{galba,
  author  = {Hoff, Katharina J. and Br{\r u}na, Tom{\'a}{\v s} and Lomsadze, Alexandre and Borodovsky, Mark and Stanke, Mario},
  title   = {A pipeline for automated gene prediction for novel species using protein homology},
  journal = {NAR Genomics and Bioinformatics},
  volume  = {5},
  number  = {3},
  pages   = {lqad064},
...
BIBEOF
            ;;
        galba2)
            echo "Hoff, K. J. (2026). GALBA2: a Snakemake pipeline for protein homology based genome annotation. In preparation." >> "$txt_file"
            cat >> "$bib_file" << 'BIBEOF'
@unpublished{galba2,
  author  = {Hoff, Katharina J.},
  title   = {{GALBA2}: a {Snakemake} pipeline for protein homology based genome annotation},
  year    = {2026},
  note    = {In preparation}
}
BIBEOF
            ;;
        miniprothint)
            echo "Bruna, T., & Li, H. (2021). miniprothint. https://github.com/tomasbruna/miniprothint" >> "$txt_file"
            cat >> "$bib_file" << 'BIBEOF'
@software{miniprothint,
  author  = {Bruna, Tomas and Li, Heng},
  title   = {miniprothint},
  year    = {2021},
  url     = {https://github.com/tomasbruna/miniprothint}
}
BIBEOF
            ;;
        trnascan)
